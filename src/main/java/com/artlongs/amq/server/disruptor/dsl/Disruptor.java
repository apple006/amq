...
import com.artlongs.amq.server.disruptor.BatchEventProcessor;
import com.artlongs.amq.server.disruptor.EventFactory;
import com.artlongs.amq.server.disruptor.EventHandler;
import com.artlongs.amq.server.disruptor.EventProcessor;
import com.artlongs.amq.server.disruptor.EventTranslator;
import com.artlongs.amq.server.disruptor.EventTranslatorOneArg;
import com.artlongs.amq.server.disruptor.EventTranslatorThreeArg;
import com.artlongs.amq.server.disruptor.EventTranslatorTwoArg;
import com.artlongs.amq.server.disruptor.ExceptionHandler;
...
import com.artlongs.amq.server.disruptor.TimeoutException;
import com.artlongs.amq.server.disruptor.WaitStrategy;
import com.artlongs.amq.server.disruptor.WorkHandler;
import com.artlongs.amq.server.disruptor.WorkerPool;
...
public class Disruptor<T>
{
    ...
    /**
     * Create a new Disruptor. Will default to {@link com.artlongs.amq.server.disruptor.BlockingWaitStrategy} and
     ...
     *
     * @param handlers the event handlers, previously set up with {@link #handleEventsWith(com.artlongs.amq.server.disruptor.EventHandler[])},
     ...
     *
     * @param processors the event processors, previously set up with {@link #handleEventsWith(com.artlongs.amq.server.disruptor.EventProcessor...)},
     *                   that will form the barrier for subsequent handlers or processors.
     * @return an {@link EventHandlerGroup} that can be used to setup a {@link SequenceBarrier} over the specified event processors.
     * @see #after(com.artlongs.amq.server.disruptor.EventHandler[])
     */
    ...
    /**
     * Calls {@link com.artlongs.amq.server.disruptor.EventProcessor#halt()} on all of the event processors created via this disruptor.
     */
    public void halt()
    {
        for (final ConsumerInfo consumerInfo : consumerRepository)
     ...
     * @see com.artlongs.amq.server.disruptor.Sequencer#getBufferSize()
     */
